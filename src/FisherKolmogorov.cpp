...
FisherKolmogorov::assemble_system()
{
          ...
          for (unsigned int q = 0; q < n_q; ++q)
            {
              if (cell->material_id() == 1)
                {
                  growth_coefficient_loc = growth_coefficient_grey.value(
                ...
                {
                  growth_coefficient_loc = growth_coefficient_white.value(
                    fe_values.quadrature_point(q));
              ...
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
                {
                  ...
                  // Growth term
                  cell_residual(i) +=
                    ((growth_coefficient_loc * solution_loc[q]) *
                     (1 - solution_loc[q])) *
   ...
   */
  void FisherKolmogorov::solve_linear_system()
  {
    // setting for solver
   ...
   * the GMRES solver.
   */
  void FisherKolmogorov::solve_newton()
  {
   ...
   */
  void FisherKolmogorov::output(const unsigned int &time_step) const
  {
